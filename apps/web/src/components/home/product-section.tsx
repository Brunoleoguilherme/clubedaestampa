...
interface Props {
  ...
  badge?: 'novo'
  /** Cor de acento da seção (rotação por seção). Padrão: azul. */
  accent?: Accent
  /** Reduz o espaço no topo (usado para encostar a seção no hero do parceiro). */
  tightTop?: boolean
}

export function ProductSection({ id, label, title, description, products, badge, accent = 'blue', tightTop }: Props) {
  if (products.length === 0) return null
  return (
    <section
      id={id}
      className={`accent-${accent} mx-auto max-w-[1440px] px-6 pb-16 sm:px-8 ${tightTop ? 'pt-4' : 'pt-16'}`}
    >
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-md">
          ...
          {description && <p className="mt-2 text-night-500">{description}</p>}
        </div>
        <Link
          href="/produtos"
